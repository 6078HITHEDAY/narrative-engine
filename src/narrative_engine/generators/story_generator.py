    ...
    def __init__(self, backend: LLMBackend | None = None) -> None:
        if backend is not None:
            ...
            if env_backend is not None:
                ...
                logger.warning(
                    "NARRATIVE_API_KEY not found; StoryGenerator is falling back to a local LLMBackend (degraded/local mode)."
                )
                self._backend = LLMBackend(max_tokens=DEFAULT_GENERATOR_MAX_TOKENS)
